...
URHO3D_DEFINE_APPLICATION_MAIN(ProjectTemplate);

ProjectTemplate::ProjectTemplate(Context* context)
    : Application(context)
{
}

void ProjectTemplate::Start()
{
    auto* cache = GetSubsystem<ResourceCache>();
    auto* graphics = GetSubsystem<Graphics>();
    graphics->SetWindowIcon(cache->GetResource<Image>("Textures/UrhoIcon.png"));
    graphics->SetWindowTitle("ProjectTemplate");
